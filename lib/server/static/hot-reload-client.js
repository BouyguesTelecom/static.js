...
(function() {
    ...
    HotReloadClient.prototype.reloadStyles = function() {
        // Reload all CSS links using create-and-swap for reliable refresh
        ...
        for (var i = 0; i < links.length; i++) {
            ...
            // Create a new <link> element so the browser performs a fresh load
            var newLink = link.cloneNode();
            newLink.href = url.toString();
            newLink.onload = function(oldLink) {
                return function() { oldLink.remove(); };
            }(link);
            newLink.onerror = function(oldLink) {
                return function() { oldLink.remove(); };
            }(link);
